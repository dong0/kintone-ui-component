...
describe("BaseDateTimeListBox", () => {
    describe("accessibility", () => {
        it("should be highlight/not highlight when mouseover/mouseleave the item", async () => {
            ...
            expect(itemsEl[2].classList.contains("kuc-base-datetime-listbox__listbox--highlight")).to.equal(true);
        });
        it("should be highlight/not highlight when mouseover/mouseleave the item", async () => {
            ...
            container.items = initItems;
            const el = await fixture(container);
            const listboxItem = el.querySelector(".kuc-base-datetime-listbox__listbox__item");
            listboxItem.focus();
        ...
        it("should be do nothing when mouseover after pressing ArrowUp key", async () => {
            ...
            const listboxItem = el.querySelector(".kuc-base-datetime-listbox__listbox__item");
            listboxItem.focus();
            listboxItem.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowUp" }));
        ...
        it("should be do nothing when mouseover after pressing ArrowDown key", async () => {
            ...
            const listboxItem = el.querySelector(".kuc-base-datetime-listbox__listbox__item");
            listboxItem.focus();
            listboxItem.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowDown" }));
            listboxItem.dispatchEvent(new Event("mouseover"));
            await elementUpdated(container);
            await elementUpdated(el);
            expect(triggeredEvent).to.equal(null);
