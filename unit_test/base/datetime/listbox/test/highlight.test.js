...
describe("BaseDateTimeListBox", () => {
    describe("hightlight", () => {
        it("should be return highlight element when call getHighlightItemEl function", async () => {
            ...
            expect(container.getHighlightItemEl().value).to.equal(2);
        });
        it("should be highlight FEBRUARY element when press ArrowDown key", async () => {
            ...
            liEl.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowDown", bubbles: true }));
            await elementUpdated(el);
            expect(container.getHighlightItemEl().value).to.equal(1);
            liEl.dispatchEvent(new KeyboardEvent("keydown", { key: "Down", bubbles: true }));
            await elementUpdated(el);
            expect(container.getHighlightItemEl().value).to.equal(2);
        });
        it("should be highlight DECEMBER element when press ArrowUp key", async () => {
            ...
            const itemsEl = el.querySelectorAll(".kuc-base-datetime-listbox__listbox")[0];
            const liEl = itemsEl.children[0];
            liEl.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowUp", bubbles: true }));
            await elementUpdated(el);
            expect(container.getHighlightItemEl().value).to.equal(2);
            liEl.dispatchEvent(new KeyboardEvent("keydown", { key: "Up", bubbles: true }));
            await elementUpdated(el);
            expect(container.getHighlightItemEl().value).to.equal(1);
        });
        it("should be highlight JANUARY/MARCH element when press Home/End key", async () => {
            ...
            const itemsEl = el.querySelectorAll(".kuc-base-datetime-listbox__listbox")[0];
            const liEl = itemsEl.children[0];
            liEl.dispatchEvent(new KeyboardEvent("keydown", { key: "Home", bubbles: true }));
            await elementUpdated(el);
            expect(container.getHighlightItemEl().value).to.equal(0);
            liEl.dispatchEvent(new KeyboardEvent("keydown", { key: "End", bubbles: true }));
            await elementUpdated(el);
            expect(container.getHighlightItemEl().value).to.equal(2);
        });
        it("should be return title null when item listbox dont have label", async () => {
            ...
            const el = await fixture(container);
            const itemsEl = el.querySelectorAll(".kuc-base-datetime-listbox__listbox__item");
            expect(itemsEl[1].title).to.equal("");
        });
        it("should be return value null when item listbox dont have value", async () => {
            ...
            const el = await fixture(container);
            const itemsEl = el.querySelectorAll(".kuc-base-datetime-listbox__listbox__item");
            expect(itemsEl[1].value).to.equal(0);
        });
