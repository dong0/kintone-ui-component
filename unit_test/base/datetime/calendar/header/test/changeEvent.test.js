...
describe("BaseDateTimeCalendarHeader", () => {
    describe("kuc:calendar-header-change event", () => {
        it("should be triggered when click the previous month button", async () => {
            ...
            const buttonEl = el.querySelector(".kuc-base-datetime-calendar-header__group__button--previous-month");
            buttonEl.click();
        ...
        it("should be triggered when press shifttab key on the previous month button", async () => {
            ...
            container.setAttribute("month", "6");
            container.setAttribute("year", "2021");
            container.addEventListener("kuc:calendar-header-previous-shifttab", (event) => {
                triggeredEvent = event;
            });
            const el = await fixture(container);
            const buttonEl = el.querySelector(".kuc-base-datetime-calendar-header__group__button--previous-month");
            buttonEl.dispatchEvent(new KeyboardEvent("keydown", { key: "Tab", shiftKey: true }));
            expect(triggeredEvent.type).to.equal("kuc:calendar-header-previous-shifttab");
        });
        it("should not be triggered when press shift + a key on the previous month button", async () => {
            ...
            const el = await fixture(container);
            const buttonEl = el.querySelector(".kuc-base-datetime-calendar-header__group__button--previous-month");
            buttonEl.dispatchEvent(new KeyboardEvent("keydown", { key: "a", shiftKey: true }));
        ...
        it("should be triggered when click the next month button", async () => {
            ...
            const el = await fixture(container);
            const buttonEl = el.querySelector(".kuc-base-datetime-calendar-header__group__button--next-month");
            buttonEl.click();
            expect(triggeredEvent.type).to.equal("kuc:calendar-header-change");
