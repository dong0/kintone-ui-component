...
describe("TimePicker", () => {
    describe("max", () => {
        it("should show all options when not assigning", async () => {
            ...
            expect(itemsEl[itemsEl.length - 1].getAttribute("value")).to.equal("23:30");
        });
        it('the last option should be 13:00 when assigned "13:15" by constructor', async () => {
            ...
            await elementUpdated(container);
            const itemsEl = el.querySelectorAll(".kuc-base-datetime-listbox__listbox__item");
            expect(itemsEl[itemsEl.length - 1].getAttribute("value")).to.equal("13:00");
        });
        ...
        it("should throw error when it is less than min", async () => {
            ...
            try {
                ...
                expect(errorMessage).to.be.equal('"max" must be greater than or equal to "min".');
            }
        });
    });
});
