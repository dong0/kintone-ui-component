...
describe("ReadOnlyTable", () => {
    describe("data", () => {
        ...
        it("should be updated rows when replaced by setter", async () => {
            ...
            const rowsEl = el.querySelectorAll(".kuc-readonly-table__table__body__row");
            expect(rowsEl.length).to.equal(2);
            expect(rowsEl[0].children[0].textContent?.trim()).to.equal(replacedData[0][0]);
            expect(rowsEl[0].children[1].textContent?.trim()).to.equal(replacedData[0][1]);
            expect(rowsEl[1].children[0].textContent?.trim()).to.equal(replacedData[1][0]);
            expect(rowsEl[1].children[1].textContent?.trim()).to.equal(replacedData[1][1]);
        });
