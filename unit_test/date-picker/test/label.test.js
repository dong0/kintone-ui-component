...
describe("DatePicker", () => {
    describe("label", () => {
        it("should be display none when not assigned in constructor", async () => {
            ...
            const labelEl = el.querySelector(".kuc-date-picker__group__label");
            expect(labelEl.hasAttribute("hidden")).to.equal(true);
            const labelTextEl = el.querySelector(".kuc-date-picker__group__label__text");
        ...
        it('should be display "options-label" when assigned "options-label" in constructor', async () => {
            const container = new DatePicker({ label: "options-label" });
            const el = await fixture(container);
            const labelEl = el.querySelector(".kuc-date-picker__group__label");
