...
describe("MobileMultiChoice", () => {
    describe("items", () => {
        ...
        it("exists on element when initializing with props option", async () => {
            ...
            expect(itemsEl[0].getAttribute("value")?.trim()).to.have.equal(expectedValues[0]);
            expect(itemsEl[1].textContent?.trim()).to.have.equal(expectedLabels[1]);
            expect(itemsEl[1].getAttribute("value")?.trim()).to.have.equal(expectedValues[1]);
            expect(itemsEl[2].textContent?.trim()).to.have.equal(expectedLabels[2]);
            expect(itemsEl[2].getAttribute("value")?.trim()).to.have.equal(expectedValues[2]);
        ...
        it("items prop replace successfully", async () => {
            ...
            expect(itemsEl[0].getAttribute("value")?.trim()).to.have.equal(itemsForReplace[0].value);
            expect(itemsEl[1].textContent?.trim()).to.have.equal(expectedLabels[2]);
            expect(itemsEl[1].getAttribute("value")?.trim()).to.have.equal(itemsForReplace[1].value);
