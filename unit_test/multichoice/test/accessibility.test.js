...
describe("MultiChoice", () => {
    describe("accessibility", () => {
        it("should be highlight/not highlight when mouseover/mouseleave the item", async () => {
            ...
            expect(itemsEl[2].classList.contains("kuc-multi-choice__group__menu__highlight")).to.equal(true);
            itemsEl[1].dispatchEvent(new Event("mouseover"));
            expect(itemsEl[2].classList.contains("kuc-multi-choice__group__menu__highlight")).to.equal(false);
            expect(itemsEl[1].classList.contains("kuc-multi-choice__group__menu__highlight")).to.equal(true);
            itemsEl[1].dispatchEvent(new Event("mouseleave"));
            expect(itemsEl[1].classList.contains("kuc-multi-choice__group__menu__highlight")).to.equal(false);
        });
        it("should not highlight/not highlight when mouseover/mouseleave the item and assinging disabled is true", async () => {
            ...
            menuEl.click();
            const itemsEl = el.querySelectorAll(".kuc-multi-choice__group__menu__item");
            const highlightEl = el.querySelector(".kuc-multi-choice__group__menu__highlight");
            itemsEl[2].dispatchEvent(new Event("mouseover"));
        ...
        it('should be highlight next item when triggered "ArrowDown" keyboard event', async () => {
            ...
            const itemsEl = el.querySelectorAll(".kuc-multi-choice__group__menu__item");
            expect(itemsEl[0].classList.contains("kuc-multi-choice__group__menu__highlight")).to.equal(true);
        });
