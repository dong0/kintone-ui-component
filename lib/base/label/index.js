...
export class BaseLabel extends KucBase {
    ...
    render() {
      ...
      <span
        class="kuc-base-label-1-3-2__required-icon"
    ...
    _getTextTemplate() {
        return this.guid && this.guid !== ""
            ? html `
          <span class="kuc-base-label-1-3-2__text" .id="${this.guid}-group"
            >${this.text}</span
          >
        `
            : html ` <span class="kuc-base-label-1-3-2__text">${this.text}</span> `;
    }
}
__decorate([
...
if (!window.customElements.get("kuc-base-label-1-3-2")) {
    createStyleOnHeader(BASE_LABEL_CSS);
    window.customElements.define("kuc-base-label-1-3-2", BaseLabel);
}
