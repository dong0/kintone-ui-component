...
export class BaseDateTimeCalendarBody extends KucBase {
    ...
    render() {
        return html `
      <table class="kuc-base-datetime-calendar-body-1-3-2__table" role="grid">
    ...
    _getDateClass(dateParts, isThisMonth) {
        if (isThisMonth) {
            ...
            if (isToday)
                return " kuc-base-datetime-calendar-body-1-3-2__table__date__button--today";
            return "";
        }
        const isToday = this._isToday(dateParts);
        if (isToday)
            return " kuc-base-datetime-calendar-body-1-3-2__table__date__button--today";
        return " kuc-base-datetime-calendar-body-1-3-2__table__date__button--other-month";
    }
      ...
      <thead>
        <tr>
          ${this._locale.WEEK_DAYS.map((wday) => {
            return html `
              <th
                class="kuc-base-datetime-calendar-body-1-3-2__table__header"
                role="columnheader"
                abbr="${wday.abbr}"
              >
                ${wday.text}
            ...
            <tr>
              ${weeks.map((weekDate) => {
                ...
                return html `
                  <td
                    role="gridcell"
                    class="kuc-base-datetime-calendar-body-1-3-2__table__date${(this
                    .value === weekDate.attr ||
                    isSameDate) &&
                    isThisMonth
                    ? "--selected"
                  ...
                  >
                    ...
                    ? "0"
                    : "-1"}"
                      class="kuc-base-datetime-calendar-body-1-3-2__table__date__button${this._getDateClass(dateParts, isThisMonth)}"
...
__decorate([
    query('.kuc-base-datetime-calendar-body-1-3-2__table__date__button[aria-current="true"]')
], BaseDateTimeCalendarBody.prototype, "_selectedItem", void 0);
__decorate([
    query(".kuc-base-datetime-calendar-body-1-3-2__table__date--selected")
], BaseDateTimeCalendarBody.prototype, "_highlightItem", void 0);
__decorate([
    query('.kuc-base-datetime-calendar-body-1-3-2__table__date__button[tabindex="0"]')
], BaseDateTimeCalendarBody.prototype, "_focusedItem", void 0);
if (!window.customElements.get("kuc-base-datetime-calendar-body-1-3-2")) {
    createStyleOnHeader(CALENDAR_BODY_CSS);
    window.customElements.define("kuc-base-datetime-calendar-body-1-3-2", BaseDateTimeCalendarBody);
}
