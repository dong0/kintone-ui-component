...
export class BaseMobileDateTimeCalendarBody extends KucBase {
    ...
    _getDateClass(dateParts, isThisMonth) {
        if (isThisMonth) {
            ...
            if (isToday)
                return " kuc-base-mobile-datetime-calendar-body-1-4-0__table__date--today";
            return "";
        }
        return " kuc-base-mobile-datetime-calendar-body-1-4-0__table__date--other-month";
    }
              ...
              ${weeks.map((weekDate) => {
                ...
                return html `
                  <td
                    role="gridcell"
                    tabindex="-1"
                    class="kuc-base-mobile-datetime-calendar-body-1-4-0__table__date${(this
                    .value === weekDate.attr ||
                    isSameDate) &&
                    isThisMonth
                    ? "--selected"
...
if (!window.customElements.get("kuc-base-mobile-datetime-calendar-body-1-4-0")) {
    createStyleOnHeader(BASE_MOBILE_CALENDAR_BODY_CSS);
    window.customElements.define("kuc-base-mobile-datetime-calendar-body-1-4-0", BaseMobileDateTimeCalendarBody);
}
