...
export class BaseMobileDateTimeCalendarBody extends KucBase {
    ...
    render() {
        return html `
      <table class="kuc-base-mobile-datetime-calendar-body-1-3-2__table" role="grid">
    ...
    _getDateClass(dateParts, isThisMonth) {
        if (isThisMonth) {
            ...
            if (isToday)
                return " kuc-base-mobile-datetime-calendar-body-1-3-2__table__date--today";
            return "";
        }
        return " kuc-base-mobile-datetime-calendar-body-1-3-2__table__date--other-month";
    }
      ...
      <thead>
        <tr>
          ${this._locale.WEEK_DAYS.map((wday) => {
            return html `
              <th
                class="kuc-base-mobile-datetime-calendar-body-1-3-2__table__header"
                role="columnheader"
                abbr="${wday.abbr}"
              >
                ${wday.text}
              ...
              ${weeks.map((weekDate) => {
                ...
                return html `
                  <td
                    role="gridcell"
                    tabindex="-1"
                    class="kuc-base-mobile-datetime-calendar-body-1-3-2__table__date${(this
                    .value === weekDate.attr ||
                    isSameDate) &&
                    isThisMonth
                    ? "--selected"
...
if (!window.customElements.get("kuc-base-mobile-datetime-calendar-body-1-3-2")) {
    createStyleOnHeader(BASE_MOBILE_CALENDAR_BODY_CSS);
    window.customElements.define("kuc-base-mobile-datetime-calendar-body-1-3-2", BaseMobileDateTimeCalendarBody);
}
