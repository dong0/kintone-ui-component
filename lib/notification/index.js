...
(() => {
    exportNotification = window.customElements.get("kuc-notification-1-3-2");
    ...
    class KucNotification extends KucBase {
      ...
      `;
        ...
        open() {
            ...
            this.performUpdate();
            this.classList.remove("kuc-notification-fadeout-1-3-2");
            this.classList.add("kuc-notification-fadein-1-3-2");
            this._isOpened = true;
            this._setAutoCloseTimer();
        }
        close() {
            this._isOpened = false;
            this.classList.remove("kuc-notification-fadein-1-3-2");
            this.classList.add("kuc-notification-fadeout-1-3-2");
            this._clearAutoCloseTimer();
        ...
        render() {
            return html `
        <div
          class="kuc-notification-1-3-2__notification kuc-notification-1-3-2__notification--${this
                .type}"
        >
          <pre
            class="kuc-notification-1-3-2__notification__title"
            aria-live="assertive"
            role="${this._isOpened ? "alert" : ""}"
          ><!--
          -->${this.text}</pre>
          <button
            class="kuc-notification-1-3-2__notification__close-button"
            type="button"
            aria-label="close"
    ...
    window.customElements.define("kuc-notification-1-3-2", KucNotification);
