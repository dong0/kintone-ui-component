...
(() => {
    exportMobileNotification = window.customElements.get("kuc-mobile-notification-1-3-2");
    ...
    class KucMobileNotification extends KucBase {
      ...
      `;
        ...
        open() {
            ...
            this.performUpdate();
            this.classList.remove("kuc-mobile-notification-fadeout-1-3-2");
            this.classList.add("kuc-mobile-notification-fadein-1-3-2");
            this._isOpened = true;
            this._setAutoCloseTimer();
        }
        close() {
            this._isOpened = false;
            this.classList.remove("kuc-mobile-notification-fadein-1-3-2");
            this.classList.add("kuc-mobile-notification-fadeout-1-3-2");
            this._clearAutoCloseTimer();
            dispatchCustomEvent(this, "close");
        }
        render() {
            return html `
        <div class="kuc-mobile-notification-1-3-2__notification">
          <pre
            class="kuc-mobile-notification-1-3-2__notification__title"
            aria-live="assertive"
            role="${this._isOpened ? "alert" : ""}"
          ><!---->${this.text}</pre>
          <button
            class="kuc-mobile-notification-1-3-2__notification__close-button"
            type="button"
            aria-label="close"
    ...
    window.customElements.define("kuc-mobile-notification-1-3-2", KucMobileNotification);
